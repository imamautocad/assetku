...
class ConsumableStockController extends Controller
{
    ...
    {
        return view('consumable.stock.index');
    }
    ...
    private function generateNoReq()
    {
        ...
        return 'CHECKIN/GA/' . $today . '/' . $sequenceFormatted;
    }
    ...
    public function create()
    {
        ...
        return view('consumable.stock.create', [
    ...
    /**
     * Simpan data order baru (draft / submit)
     */
    public function store(Request $request)
    {
        $request->validate([
            ...
            'status'                => $item['status'] ?? 'checkin', // Tambahan status
        ]);
        ...
        try {
            ...
            foreach ($request->items as $item) {
                ...
                // Validasi stok bila status submit
                if ($request->action === 'submit') {
                    // if ($consumable->qty < $item['qty']) {
                    //     throw new \Exception("Stok item {$consumable->name} tidak mencukupi. Stok tersisa: {$consumable->qty}");
                    // }

                    // Kurangi stok
                    $consumable->increment('qty', $item['qty']);

                    // Tambahkan ke tabel consumable_users
                    ConsumablesUsers::create([
                     ...
                     // Tambahkan log ke action_logs
                    ActionLog::create([
                        'created_by'   => Auth::id(),
                        'action_type'  => 'checkin',
                        'target_id'    => Auth::id(),
                        'target_type'  => 'App\\Models\\User',
                        'note'         => ($request->notes ?? 'Request consumable submitted') . ' | PO: ' . ($request->no_po ?? '-'),
            ...
            return redirect()->route('consumable.stock.index')->with(
                'success',
                $request->action === 'submit'
                    ? 'Request berhasil disubmit, stok diperbarui dan data pengguna tersimpan!'
    ...
    public function show($id)
    {
        ...
        return view('consumable.stock.show', compact('order'));
    }
    ...
    public function edit($id)
    {
        ...
        if ($order->status !== 'draft') {
            return redirect()->route('consumable.stock.index')->with('error', 'Order tidak dapat diedit karena status bukan draft.');
        }

        $consumables = Consumable::whereNull('deleted_at')->where('qty', '>', 0)->get();
        return view('consumable.stock.edit', compact('order', 'consumables'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            foreach ($request->details as $item) {
                ...
                $consumable = Consumable::find($item['consumable_id']);
                if (!$consumable) {
                    throw new \Exception('Consumable tidak ditemukan.');
                }
                ...
                if ($request->action === 'submit') {
                    ...
                    ActionLog::create([
                        ...
                        'target_id'    => Auth::id(),
                        'target_type'  => 'App\\Models\\User',
                        'note'          => ($request->notes ?? 'Add consumable submitted') . ' | PO: ' . ($request->no_po ?? '-'),
                        'item_type'    => 'App\\Models\\Consumable',
                        'item_id'      => $consumable->id,
                        'action_date'  => now(),
                        'action_source'=> 'gui',
                        'remote_ip'    => FacadeRequest::ip(),
                        'user_agent'   => FacadeRequest::header('User-Agent'),
                    ]);
                }
            ...
            // Update status
            $order->update(['status' => $request->action === 'submit' ? 'submitted' : 'draft']);
    ...
    public function print($id)
    {
        ...
        $pdf = Pdf::loadView('pdf.consumable-stock', [
            'order' => $order,
            'logo' =>  $logoBase64
        ]);
        ...
        return $pdf->stream('consumable-stock-'.$order->no_req.'.pdf');
    }
}
