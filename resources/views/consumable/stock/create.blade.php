        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">Add Consumable Stock</h3>
                </div>

                <form action="{{ route('consumable.stock.store') }}" method="POST" id="consumableForm">
                    @csrf
                    <div class="box-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label>No. PO <small>(Purchase Order)</small></label>
                                    <input type="text" class="form-control input-sm" name="no_po" required>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                            ...
                            <table class="table table-bordered table-striped" id="detailTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 15%">Qty (Tambah)</th>
                                    ...
                                    <tr>
                                            ...
                                            <select name="items[0][consumable_id]" class="form-control input-sm select2" required>
                                                ...
                                                @foreach ($consumables as $item)
                                                    <option value="{{ $item->id }}">
                                                        {{ $item->name }} (Stok Saat Ini: {{ $item->qty }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <button type="button" class="btn btn-success btn-sm addRow">
                                                <i class="bi bi-plus-circle-fill" style="font-size:1.3rem;"></i>
                                            </button>
                                        </td>
                                    </tr>
                                </tbody>
                    ...
                    <div class="box-footer">
                        ...
                        <a href="{{ route('consumable.stock.index') }}" class="btn btn-default btn-sm">
            ...
            newRow.innerHTML = `
                <td>
                    <select name="items[${rowIndex}][consumable_id]" class="form-control input-sm select2" required>
                        ...
                        @foreach ($consumables as $item)
                            <option value="{{ $item->id }}">{{ $item->name }} (Stok Saat Ini: {{ $item->qty }})</option>
                        @endforeach
                    </select>
                </td>
