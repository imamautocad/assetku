...
{{-- Page title --}}
@section('title')
Consumable Stock Orders 
@parent
@stop

@section('header_right')
  <a href="{{ route('consumable.stock.create') }}" {{$snipeSettings->shortcuts_enabled == 1 ? "accesskey=n" : ''}} class="btn btn-primary pull-right">
    ...
    <div class="box box-default">
      <div class="box-body">
        ...
        <table
            ...
            data-toggle="table"
            data-url="{{ route('api.consumable.stock.index') }}"
            ...
            data-cookie-id-table="consumableStockTable"
            ...
            <thead>
                <tr>
                    ...
                    <th data-field="no_po" data-sortable="true">PO.No</th>
...
<script>
    ...
    function actionFormatter(value, row) {
        ...
        html += '<a href="{{ url("consumable-stock") }}/' + row.id + '" class="btn btn-xs btn-info" title="View"><i class="bi bi-eye-fill"></i></a> ';

        // Edit: only if API sets can_edit = true
        if (row.can_edit) {
            html += '<a href="{{ url("consumable-stock") }}/' + row.id + '/edit" class="btn btn-xs btn-warning" title="Edit"><i class="bi bi-pencil-square"></i></a> ';
        } 

        // Print: show when status submitted (case-insensitive check)
        if (row.status && row.status.toLowerCase() === 'submitted') {
            html += '<a target="_blank" href="{{ url("consumable-stock/print") }}/' + row.id + '" class="btn btn-xs btn-primary" title="Print"><i class="bi bi-printer-fill""></i></a>';
        }
